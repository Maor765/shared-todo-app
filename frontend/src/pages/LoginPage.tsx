...
export default function LoginPage() {
  ...
  const inputStyle = { width: '100%', height: 40, borderRadius: 10, background: 'var(--bg-card)', border: '0.5px solid var(--border)', padding: '0 12px', fontSize: 14, marginBottom: 12, outline: 'none', color: 'var(--text)' } as const;
  ...
  return (
    <div style={{ width: '100%', maxWidth: 480, minHeight: '100dvh', background: 'var(--bg)', display: 'flex', flexDirection: 'column', position: 'relative' }}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', padding: '40px 24px', textAlign: 'center' }}>
        ...
        <p style={{ fontSize: 14, color: 'var(--text-muted)', marginBottom: 32 }}>{t('sign_in_sub')}</p>
