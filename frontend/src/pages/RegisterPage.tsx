...
export default function RegisterPage() {
  ...
  const inputStyle = { width: '100%', height: 40, borderRadius: 10, background: 'var(--bg-card)', border: '0.5px solid var(--border)', padding: '0 12px', fontSize: 15, marginBottom: 10, outline: 'none', color: 'var(--text)' } as const;
  ...
  return (
    <div style={{ width: '100%', maxWidth: 480, minHeight: '100dvh', background: 'var(--bg)', display: 'flex', flexDirection: 'column', position: 'relative' }}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', padding: '40px 24px', textAlign: 'center' }}>
        ...
        <p style={{ fontSize: 15, color: 'var(--text-muted)', marginBottom: 32 }}>{t('create_account_sub')}</p>
        ...
        <div style={{ position: 'absolute', inset: 0, background: 'rgba(26,26,26,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 32, zIndex: 10, padding: 28 }}>
          <div style={{ background: 'var(--bg-card)', borderRadius: 20, padding: '32px 24px', width: '100%', textAlign: 'center', boxShadow: '0 4px 24px rgba(0,0,0,0.12)' }}>
            ...
            <div style={{ fontSize: 15, color: 'var(--text-muted)', lineHeight: 1.6, marginBottom: 28 }}>
              {t('invite_message')}<br />
              <strong style={{ color: 'var(--text)', fontSize: 16 }}>{pendingInvite.workspace_name}</strong>
              {' '}{t('invite_as')} <strong style={{ color: 'var(--text)' }}>{pendingInvite.role}</strong>.
            </div>
            <button onClick={handleAccept} disabled={accepting}
              style={{ width: '100%', height: 42, borderRadius: 10, background: 'var(--primary)', color: '#fff', border: 'none', fontSize: 16, fontWeight: 600, cursor: 'pointer', marginBottom: 10, opacity: accepting ? 0.6 : 1 }}>
              {accepting ? t('joining') : `${t('join_workspace')} ${pendingInvite.workspace_name}`}
            </button>
            <button onClick={() => { auth.finalizeSession(); navigate({ to: '/' }); }}
              style={{ width: '100%', height: 42, borderRadius: 10, background: 'var(--bg)', color: 'var(--text-muted)', border: '0.5px solid var(--border)', fontSize: 15, cursor: 'pointer' }}>
              {t('skip_invite')}
            </button>
          </div>
        </div>
