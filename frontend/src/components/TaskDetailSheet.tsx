...
export default function TaskDetailSheet({ task, listId, onClose, onSave, onDelete }: TaskDetailSheetProps) {
  ...
  const inputStyle = { width: '100%', height: 36, borderRadius: 10, background: 'var(--bg-input)', border: '0.5px solid var(--border)', padding: '0 8px', fontSize: 14, outline: 'none', color: 'var(--text)' } as const;
    ...
    <Sheet open={true} onClose={onClose}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
        ...
        <input value={text} onChange={(e) => setText(e.target.value)}
          style={{ flex: 1, fontSize: 17, fontWeight: 600, color: 'var(--text)', border: 'none', outline: 'none', background: 'none' }} />
      </div>

      <div style={{ marginBottom: 12 }}>
        <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 4, fontWeight: 600 }}>{t('notes_label')}</div>
        <textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder={t('notes_ph')} rows={3}
          style={{ width: '100%', borderRadius: 10, background: 'var(--bg-input)', border: '0.5px solid var(--border)', padding: '8px 12px', fontSize: 14, outline: 'none', resize: 'none', color: 'var(--text)' }} />
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 12 }}>
        <div>
          <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 4, fontWeight: 600 }}>{t('assignee_label')}</div>
        ...
        <div>
          <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 4, fontWeight: 600 }}>{t('due_date')}</div>
      ...
      {(list.sublists || []).length > 0 && (
        <div style={{ marginBottom: 14 }}>
          <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 4, fontWeight: 600 }}>{t('sublist_label')}</div>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        <button onClick={handleSave} disabled={saving}
          style={{ flex: 1, padding: 12, borderRadius: 10, background: 'var(--primary)', color: '#fff', border: 'none', fontSize: 15, fontWeight: 600, cursor: 'pointer', opacity: saving ? 0.6 : 1 }}>
