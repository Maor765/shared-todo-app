    ...
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '4px 16px 16px' }}>
        {current && (
          <div style={{ background: 'var(--bg-card)', borderRadius: 14, border: '0.5px solid var(--border)', padding: '20px 16px', textAlign: 'center', marginBottom: 14 }}>
            <div style={{ margin: '0 auto 10px', width: 64, height: 64, borderRadius: '50%', background: current.color, color: current.text_color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 26, fontWeight: 700 }}>
              {current.initials}
            </div>
            <div style={{ fontSize: 21, fontWeight: 700, color: 'var(--text)' }}>{current.name}</div>
            <div style={{ fontSize: 16, color: 'var(--text-muted)', marginTop: 3 }}>{current.email}</div>
            ...
            <button onClick={auth.logout}
              style={{ marginTop: 14, padding: '8px 20px', borderRadius: 10, border: '0.5px solid var(--border)', background: 'var(--bg-card)', color: 'var(--danger)', fontSize: 16, fontWeight: 600, cursor: 'pointer' }}>
              {t('log_out')}
            </button>
          </div>
        )}
          ...
          {members.map((m, i) => (
            <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 0', borderBottom: i < members.length - 1 ? '0.5px solid var(--border-subtle)' : 'none' }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 17, fontWeight: 600, color: 'var(--text)' }}>{m.name}</div>
                <div style={{ fontSize: 14, color: 'var(--text-muted)', marginTop: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{m.email}</div>
                <div style={{ fontSize: 14, color: 'var(--text-faint)', marginTop: 1, textTransform: 'capitalize' }}>
        ...
        {invites.length > 0 && (
          <>
            <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-faint)', textTransform: 'uppercase', letterSpacing: 0.6, margin: '18px 0 8px' }}>
            ...
            <div style={{ background: 'var(--bg-card)', borderRadius: 14, border: '0.5px solid var(--border)', padding: '0 14px' }}>
              {invites.map((invite, i) => (
                <div key={invite.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '11px 0', borderBottom: i < invites.length - 1 ? '0.5px solid var(--border-subtle)' : 'none' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 16, fontWeight: 600, color: 'var(--text)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{invite.email}</div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 3 }}>
                      <Badge variant="warn">{t('pending_badge')}</Badge>
                      <span style={{ fontSize: 14, color: 'var(--text-faint)', textTransform: 'capitalize' }}>{invite.role === 'admin' ? t('admin_role') : t('member_role')}</span>
                    </div>
                  </div>
        ...
        {invited && (
          <div style={{ marginTop: 12, padding: '10px 14px', background: 'var(--success-bg)', borderRadius: 10, fontSize: 16, color: 'var(--success-dim)', fontWeight: 500 }}>
            {t('invite_sent')}
          </div>
        )}
      </div>
      ...
      <Sheet open={showInvite} onClose={() => { setShowInvite(false); setInvited(false); }} title={t('invite_to_ws')}>
        <div style={{ marginBottom: 14 }}>
          <div style={{ fontSize: 14, color: 'var(--text-muted)', marginBottom: 4, fontWeight: 600 }}>{t('email_address')}</div>
          <input value={inviteEmail} onChange={(e) => setInviteEmail(e.target.value)} placeholder="colleague@company.com"
            style={{ width: '100%', height: 38, borderRadius: 10, background: 'var(--bg-input)', border: '0.5px solid var(--primary)', padding: '0 12px', fontSize: 17, outline: 'none', color: 'var(--text)' }} />
        </div>

        <div style={{ marginBottom: 16 }}>
          <div style={{ fontSize: 14, color: 'var(--text-muted)', marginBottom: 8, fontWeight: 600 }}>{t('role_label')}</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            {(['member', 'admin'] as const).map((r) => (
              <button key={r} onClick={() => setInviteRole(r)}
                ...
                <div style={{ fontSize: 16, fontWeight: 600, color: inviteRole === r ? 'var(--primary)' : 'var(--text)', textTransform: 'capitalize' }}>
                  {r === 'member' ? t('member_role') : t('admin_role')}
                </div>
                <div style={{ fontSize: 14, color: inviteRole === r ? 'var(--primary-dim)' : 'var(--text-muted)', marginTop: 2 }}>
        ...
        <button onClick={handleInvite} disabled={inviteMutation.isPending || !inviteEmail}
          style={{ width: '100%', padding: 13, borderRadius: 10, background: 'var(--primary)', color: '#fff', border: 'none', fontSize: 17, fontWeight: 600, cursor: 'pointer', opacity: inviteMutation.isPending || !inviteEmail ? 0.6 : 1 }}>
